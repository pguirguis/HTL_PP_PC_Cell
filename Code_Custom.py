...
def ode1_PP(x, tspan, k):

    try:
        k[1]
    except:
        k = [item for subl in k for item in subl]    
...
def Fit_Fun1_PP(tspan,data,g,s):
    ...
    def my_ls_func1(tspan,*teta):
        ...
        f2 = lambda t,y: ode1_PP(y, t, teta)
    ...
    pred = my_ls_func1(tspan,k)
    pred = pred.astype(float)
    
    data = data.astype(float)
    
    res = np.sum(np.sum(np.subtract(data,pred)**2))
    
    return k , var ,res

def Fit_Fun1_PC(tspan,data,g,s):
    ...
    def my_ls_func1(tspan,*teta):
        """definition of function for LS fit
            ...
            teta is an array of parameters to be varied for fit"""
        # create an alias to f which passes the optional params    
        f2 = lambda t,y: ode1_PC(y, t, teta)
        # calculate ode solution, retuen values for each entry of "x"
...
def Fit_Fun1_Cell(tspan,data,g,s):
    ...
    def my_ls_func1(tspan,*teta):
        """definition of function for LS fit
            ...
            teta is an array of parameters to be varied for fit"""
        # create an alias to f which passes the optional params    
        f2 = lambda t,y: ode1_Cell(y, t, *teta)
        # calculate ode solution, retuen values for each entry of "x"
...
PP_kf,PP_k_var, PP_res = Fit_Fun1_PP(t[:-1],PP_data0,np.random.rand(k_n1)*0.00001,PP_sigma0)
PP_sd = np.sqrt(np.diag(PP_k_var))
PC_kf,PC_k_var,PC_res = Fit_Fun1_PC(t,PC_data0,np.random.rand(k_n1)*0.00001,PC_sigma0)
PC_sd = np.sqrt(np.diag(PC_k_var))
Cell_kf,Cell_k_var,Cell_res = Fit_Fun1_Cell(t,Cell_data0,np.random.rand(k_n1)*0.0001,Cell_sigma0)
Cell_sd = np.sqrt(np.diag(Cell_k_var))


def Fit_Fun2_PC_PP(datain,dataout,g,ks,s):
    ...
    def my_ls_func2(per,*teta):
        """definition of function for LS fit
            ...
            teta is an array of parameters to be varied for fit"""
        # create an alias to f which passes the optional params    
        f2 = lambda t,y: ode2_PC_PP(y, t,ks,teta)
        # calculate ode solution, retuen values for each entry of "x"
...
def Fit_Fun2_PP_Cell(datain,dataout,g,ks,s):
    ...
    def my_ls_func2(per,*teta):
        """definition of function for LS fit
            ...
            teta is an array of parameters to be varied for fit"""
        # create an alias to f which passes the optional params    
        f2 = lambda t,y: ode2_PP_Cell(y, t,ks,teta)
        # calculate ode solution, retuen values for each entry of "x"
        pred = np.zeros(len(per))
        for q in range(len(per)):
            data0 = [per[q],100-per[q], 0, 0, 0, 0, 0, 0, 0, 0, 0]
...
def Fit_Fun2_PC_Cell(datain,dataout,g,ks,s):
    ...
    def my_ls_func2(per,*teta):
        """definition of function for LS fit
            ...
            teta is an array of parameters to be varied for fit"""
        # create an alias to f which passes the optional params    
        f2 = lambda t,y: ode2_PC_Cell(y, t,ks,teta)
        # calculate ode solution, retuen values for each entry of "x"
        pred = np.zeros(len(per))
        for q in range(len(per)):
            data0 = [per[q],100-per[q], 0, 0, 0, 0, 0, 0, 0, 0, 0]
...
PP_Cell_power,PP_Cell_power_var= Fit_Fun2_PP_Cell(PP_Cell_percent,PP_Cell_B,[2.616066575192726659e-02,-1.027797108998573350e+00,2.610809385178870290e+00,-1.735412956399966511e-01,1.764540561794312179e-01],[PP_kf,Cell_kf],PP_Cell_sigma)
PP_Cell_power_sd = np.sqrt(np.diag(PP_Cell_power_var))
PC_Cell_power,PC_Cell_power_var = Fit_Fun2_PC_Cell(PC_Cell_percent,PC_Cell_B,np.zeros(k_n2),[PC_kf,Cell_kf],PC_Cell_sigma)
PC_Cell_power_sd = np.sqrt(np.diag(PC_Cell_power_var))
PC_PP_power,PC_PP_power_var = Fit_Fun2_PC_PP(PC_PP_percent,PC_PP_B,[-0.00388229,1.21728,0.398988,2.68953,-0.326792],[PC_kf,PP_kf],PC_PP_sigma)
...
def Fit_Fun3_PP_PC(tspan,data,ks,g,frac,s):
    row,col = np.shape(data)
    t0 = 0
    tf = tspan[-1]
    tspan = np.asarray(tspan, dtype = np.float64, order ='C')
    ...
    def my_ls_func3(tspan,*teta):
        """definition of function for LS fit
            ...
            teta is an array of parameters to be varied for fit"""
        # create an alias to f which passes the optional params    
        f2 = lambda t,y: ode3_PP_PC(y, t, ks ,teta)
        # calculate ode solution, retuen values for each entry of "x"
...
def Fit_Fun3_PP_Cell(tspan,data,ks,g,frac,s):
    row,col = np.shape(data)
    t0 = 0
    tf = tspan[-1]
    tspan = np.asarray(tspan, dtype = np.float64, order ='C')
    ...
    def my_ls_func3(tspan,*teta):
        """definition of function for LS fit
            ...
            teta is an array of parameters to be varied for fit"""
        # create an alias to f which passes the optional params    
        f2 = lambda t,y: ode3_PP_Cell(y, t, ks ,teta)
        # calculate ode solution, retuen values for each entry of "x"
...
def Fit_Fun3_PC_Cell(tspan,data,ks,g,frac,s):
    row,col = np.shape(data)
    t0 = 0
    tf = tspan[-1]
    tspan = np.asarray(tspan, dtype = np.float64, order ='C')
    ...
    def my_ls_func3(tspan,*teta):
        """definition of function for LS fit
            ...
            teta is an array of parameters to be varied for fit"""
        # create an alias to f which passes the optional params    
        f2 = lambda t,y: ode3_PC_Cell(y, t, ks ,teta)
        # calculate ode solution, retuen values for each entry of "x"
...
PP_PC_kf, PP_PC_var = Fit_Fun3_PP_PC(t,PP_PC_data0,k_PP_PC,np.zeros(k_n3),0.5,PP_PC_sigma0)
PP_PC_sd = np.sqrt(np.diag(PP_PC_var))
PC_Cell_kf, PC_Cell_var = Fit_Fun3_PC_Cell(t,PC_Cell_data0,k_PC_Cell,np.zeros(k_n3),.8,PC_Cell_sigma0)
PC_Cell_sd = np.sqrt(np.diag(PC_Cell_var))
PP_Cell_kf, PP_Cell_var = Fit_Fun3_PP_Cell(t,PP_Cell_data0,k_PP_Cell,np.zeros(k_n3),.5,PP_Cell_sigma0)
PP_Cell_sd = np.sqrt(np.diag(PP_Cell_var))




def Plot_Fun1_PP(tspan,k):
    t0 = tspan[0]
    tf = tspan[-1]
...
PP_fitf, PP_fit2 = Plot_Fun1_PP(tfit, PP_kf)
PC_fitf, PC_fit2 = Plot_Fun1_PC(tfit, PC_kf)
Cell_fitf, Cell_fit2 = Plot_Fun1_Cell(tfit, Cell_kf)
...
PC_PP_fit, PC_PP_fit2 = Plot_Fun2_PC_PP(k_PC_PP, PC_PP_percent)
PC_Cell_fit, PC_Cell_fit2 = Plot_Fun2_PC_Cell(k_PC_Cell, PC_Cell_percent)
PP_Cell_fit, PP_Cell_fit2 = Plot_Fun2_PP_Cell(k_PP_Cell, PP_Cell_percent)
...
def Plot_Fun3_PP_PC(tspan,ks,frac):
    ks = [item for subl in ks for item in subl]
    t0 = 0
    tf = tspan[-1]
...
def Plot_Fun3_PP_Cell(tspan,ks,frac):
    ks = [item for subl in ks for item in subl]
    t0 = 0
    tf = tspan[-1]
...
def Plot_Fun3_PC_Cell(tspan,ks,frac):
    ks = [item for subl in ks for item in subl]
    t0 = 0
    tf = tspan[-1]
...
PP_PC_fit, PP_PC_fit2 = Plot_Fun3_PP_PC(tfit,PP_PC_full,.5)
PC_Cell_fit, PC_Cell_fit2 = Plot_Fun3_PC_Cell(tfit,PC_Cell_full,.8)
PP_Cell_fit, PP_Cell_fit2 = Plot_Fun3_PP_Cell(tfit,PP_Cell_full,.5)
...
PC_PP_fit , PC_PP_fit2 = Plot_Fun2_PC_PP(k_PC_PP,PC_PP_percent)
PC_Cell_fit , PC_Cell_fit2 = Plot_Fun2_PC_Cell(k_PC_Cell,PC_Cell_percent)
PP_Cell_fit , PP_Cell_fit2 = Plot_Fun2_PP_Cell(k_PP_Cell,PC_Cell_percent)
