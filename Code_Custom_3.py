...
PP_kf = [2.257304531558238739e-03,4.207600911018753997e-01,5.548716660364945765e-18,8.379239694069165068e-14,4.690059102342392094e-01,1.610748027232430657e-01,1.072561128171016537e-15,7.715648511242721108e-15,8.055592701134050215e-04,4.030506997189653711e-13,3.509879676376756704e-14]
PP_sd = np.zeros(k_n1)
PC_kf = [1.739895888465098350e-01,1.129757606465108281e-03,1.908641759536983855e-03,3.007957105378209982e-13,4.836041875072342692e-14,3.692311912018915888e-03,2.132600273289174744e-02,4.303063562864910263e-02,7.586912561651012895e-18,1.103968622279574317e-03,2.496724662853312905e-12]
PC_sd = np.zeros(k_n1)
Cell_kf = [4.474952417726574608e-03,
5.477225708090288636e-02,
3.746694960231664040e-02,
2.668952933412890129e-17,
2.128952621857886085e-01,
1.869364231089864755e-22,
2.288553620129509317e-02,
2.268242402217656872e-23,
2.664454387932304116e-02,
1.751462716624091129e-02,
3.041881735897417290e-18]
Cell_sd = np.zeros(k_n1)



PP_Cell_power = [2.616066575192726659e-02,-1.027797108998573350e+00,2.610809385178870290e+00,-1.735412956399966511e-01,1.764540561794312179e-01]
PP_Cell_power_sd = np.zeros(k_n2)
PC_Cell_power = [0,1,1,1,1]
PC_Cell_power_sd = np.zeros(k_n2)
PC_PP_power = [-0.00388229,1.21728,0.398988,2.68953,-0.326792]
PC_PP_power_sd = np.zeros(k_n2)
...
PP_PC_kf = [0.020302272,0.000,0.050000799,0.010011988,-6.43507E-06,-6.90542E-07,-2.9865E-05,0.050010823,0.0]
PP_PC_sd = np.zeros(k_n3)
PC_Cell_kf = [0.003,0.003,0.03,0.1,0.002,0.01,0.001,0.07,0.02]
PC_Cell_sd = np.zeros(k_n3)
PP_Cell_kf = [0.001,0.001,0.03,0,0,0,0.001,0.0,0.1]
PP_Cell_sd = np.zeros(k_n3)
